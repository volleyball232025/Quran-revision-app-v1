import {loadState,saveState,resetState,captureRecoverySnapshot,findRecoverySnapshot,restoreRecoverySnapshot} from './storage.js';
import {fetchSurahCatalog,fetchSurahVerses,fetchAudioEditions,fetchSurahAudio,fetchWordByWordSurah,fetchQcfMushafPage,BISMILLAH,hasBismillah} from './quran-provider.js';
import {stars,shortToday,strengthScore,escapeHtml} from './utils.js';
import {fallbackCatalog,initialCollection,initialRevision} from './demo-data.js';
...
const CLOUD_LOCAL_UPDATED_KEY='quran-revision-local-updated-v1';
const CLOUD_LAST_SYNC_KEY='quran-revision-last-cloud-sync-v1';
const CLOUD_LAST_USER_KEY='quran-revision-last-cloud-user-v1';
const cloudApi=window.QuranCloud||{available:false};
let cloudSession=null;
let cloudStatus=cloudApi.available?'signed-out':'unavailable';
let cloudLastSyncedAt=Number(localStorage.getItem(CLOUD_LAST_SYNC_KEY))||0;
let cloudSyncTimer=0;
let cloudApplying=false;
let cloudReconciling=false;
let passwordRecoveryMode=false;
if(!localStorage.getItem(CLOUD_LOCAL_UPDATED_KEY))localStorage.setItem(CLOUD_LOCAL_UPDATED_KEY,String(Date.now()));
...
function cloudStatusLabel(){if(!cloudApi.available)return 'Local';if(!cloudSession)return 'Local';if(cloudStatus==='syncing')return 'Syncing';if(cloudStatus==='error')return 'Offline';return 'Cloud'}
function cloudStatusDescription(){
  if(!cloudApi.available)return 'Local saving is active. Cloud services could not load.';
  if(!cloudSession)return 'Saved locally on this device. Sign in to sync across devices.';
  if(cloudStatus==='syncing')return `Syncing ${cloudUserEmail()||'your account'}…`;
  if(cloudStatus==='error')return `Signed in as ${cloudUserEmail()||'your account'}, but the last cloud sync failed. Local saving is still active.`;
  const when=cloudLastSyncedAt?new Date(cloudLastSyncedAt).toLocaleString([],{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}):'just now';
  return `Signed in as ${cloudUserEmail()||'your account'} · last cloud sync ${when}`;
}
function markLocalUpdated(){try{localStorage.setItem(CLOUD_LOCAL_UPDATED_KEY,String(Date.now()))}catch{}}
function localUpdatedAt(){return Number(localStorage.getItem(CLOUD_LOCAL_UPDATED_KEY))||0}
function rememberCloudSync(userId,when=Date.now()){
  cloudLastSyncedAt=Number(when)||Date.now();
  try{localStorage.setItem(CLOUD_LAST_SYNC_KEY,String(cloudLastSyncedAt));localStorage.setItem(CLOUD_LAST_USER_KEY,String(userId||''))}catch{}
}
function normalizeIncomingState(raw){
  const incoming=raw&&typeof raw==='object'?raw:{};
  const merged={...defaultState,...incoming,
    profile:{...defaultState.profile,...(incoming.profile||{})},
    stats:{...defaultState.stats,...(incoming.stats||{})},
    meanings:{...defaultState.meanings,...(incoming.meanings||{}),quiz:{...defaultState.meanings.quiz,...(incoming.meanings?.quiz||{})},performance:{...(incoming.meanings?.performance||{})}},
    listen:{...defaultState.listen,...(incoming.listen||{})}
  };
  merged.profile.displayName=String(merged.profile?.displayName||'').trim().slice(0,40);
  merged.collection=Array.isArray(merged.collection)?merged.collection.map(Number).filter(n=>n>=1&&n<=114):[];
  merged.revision=merged.revision&&typeof merged.revision==='object'?merged.revision:{};
  merged.catalog=Array.isArray(merged.catalog)&&merged.catalog.length?merged.catalog:fallbackCatalog;
  merged.stats.redJuz=Array.isArray(merged.stats.redJuz)?merged.stats.redJuz.map(Number).filter(n=>n>=1&&n<=30):[];
  merged.meanings.known=Array.isArray(merged.meanings.known)?[...new Set(merged.meanings.known)]:[];
  merged.meanings.learnedOn=merged.meanings.learnedOn&&typeof merged.meanings.learnedOn==='object'?merged.meanings.learnedOn:{};
  merged.listen.favoriteReciters=Array.isArray(merged.listen.favoriteReciters)?merged.listen.favoriteReciters:[];
  merged.listen.highlights=merged.listen.highlights&&typeof merged.listen.highlights==='object'?merged.listen.highlights:{};
  return merged;
}
function scheduleCloudSync(){
  if(!cloudApi.available||!cloudSession||cloudApplying||cloudReconciling)return;
  clearTimeout(cloudSyncTimer);cloudSyncTimer=setTimeout(()=>syncCloudState(),1400);
}
async function syncCloudState({announceResult=false}={}){
  if(!cloudApi.available||!cloudSession||cloudApplying||cloudReconciling)return false;
  clearTimeout(cloudSyncTimer);cloudStatus='syncing';
  if(state.mode==='practice'&&state.screen==='profile')render();
  try{
    await cloudApi.saveState(cloudSession.user.id,state);
    rememberCloudSync(cloudSession.user.id,Date.now());
    cloudStatus='synced';
    if(announceResult)announce('Cloud sync complete.');else if(state.mode==='practice'&&state.screen==='profile')render();
    return true;
  }catch(err){
    cloudStatus='error';
    if(announceResult)announce('Cloud sync failed. Your changes are still saved locally.');else if(state.mode==='practice'&&state.screen==='profile')render();
    console.warn('Quran Revision cloud sync failed',err);return false;
  }
}
async function useCloudRecord(row){
  cloudApplying=true;
  try{
    captureRecoverySnapshot({force:true,label:'before-cloud-load'});
    state=normalizeIncomingState(row?.state||{});
    saveState(state);
    const cloudMs=Date.parse(row?.updated_at||'')||Date.now();
    try{localStorage.setItem(CLOUD_LOCAL_UPDATED_KEY,String(cloudMs))}catch{}
    rememberCloudSync(cloudSession?.user?.id,cloudMs);
    cloudStatus='synced';render();
    if(state.mode==='practice'&&state.screen==='meanings')ensureMeaningBank();
  }finally{cloudApplying=false}
}
function maybeAskPreferredName(){
  if(!cloudSession||state.profile.displayName||document.querySelector('.preferred-name-setup'))return;
  const explicit=String(cloudSession.user?.user_metadata?.preferred_name||'').trim().slice(0,40);
  if(explicit){state.profile.displayName=explicit;persist();return}
  setTimeout(()=>{if(cloudSession&&!state.profile.displayName&&!document.querySelector('.preferred-name-setup')){document.body.insertAdjacentHTML('beforeend',preferredNameSetupSheet());bindPreferredNameSetup()}},150);
}
async function reconcileCloudSession(session){
  if(!cloudApi.available||!session||cloudReconciling)return;
  cloudReconciling=true;cloudStatus='syncing';
  if(state.mode==='practice'&&state.screen==='profile')render();
  try{
    const row=await cloudApi.loadState(session.user.id);
    const lastUser=localStorage.getItem(CLOUD_LAST_USER_KEY)||'';
    if(!row){
      await cloudApi.saveState(session.user.id,state);rememberCloudSync(session.user.id,Date.now());cloudStatus='synced';
    }else if(lastUser===session.user.id){
      const cloudMs=Date.parse(row.updated_at||'')||0,localMs=localUpdatedAt();
      if(localMs>cloudMs+3000){await cloudApi.saveState(session.user.id,state);rememberCloudSync(session.user.id,Date.now());cloudStatus='synced'}
      else if(cloudMs>localMs+3000){await useCloudRecord(row)}
      else{rememberCloudSync(session.user.id,Math.max(cloudMs,Date.now()));cloudStatus='synced'}
    }else{
      const useCloud=confirm('Cloud data already exists for this account.\n\nPress OK to load the cloud copy on this device.\nPress Cancel to keep this device copy and replace the cloud copy.\n\nA local recovery snapshot will be created before either choice.');
      captureRecoverySnapshot({force:true,label:'before-first-cloud-choice'});
      if(useCloud)await useCloudRecord(row);else{await cloudApi.saveState(session.user.id,state);rememberCloudSync(session.user.id,Date.now());cloudStatus='synced'}
    }
  }catch(err){cloudStatus='error';console.warn('Quran Revision cloud load failed',err)}
  finally{cloudReconciling=false;if(state.mode==='practice'&&state.screen==='profile')render();maybeAskPreferredName()}
}
async function initCloudAuth(){
  if(!cloudApi.available)return;
  try{
    cloudApi.onAuthStateChange((event,session)=>{
      cloudSession=session||null;
      if(event==='PASSWORD_RECOVERY'){passwordRecoveryMode=true;setTimeout(()=>{document.body.insertAdjacentHTML('beforeend',passwordRecoverySheet());bindPasswordRecoverySheet()},120)}
      if(session){reconcileCloudSession(session)}else{cloudStatus='signed-out';if(state.mode==='practice'&&state.screen==='profile')render()}
    });
    const {data,error}=await cloudApi.getSession();if(error)throw error;
    cloudSession=data?.session||null;
    if(cloudSession)await reconcileCloudSession(cloudSession);else cloudStatus='signed-out';
  }catch(err){cloudStatus='error';console.warn('Supabase auth initialization failed',err)}
}
function preferredNameSetupSheet(){return `<div class="modal-backdrop preferred-name-setup"><section class="sheet compact-sheet" role="dialog" aria-modal="true"><div class="sheet-head"><div><h2 class="title">Welcome</h2><p class="subtitle">One quick account detail</p></div></div><label class="mem-field"><span>What should we call you?</span><input id="setup-preferred-name" class="search" maxlength="40" placeholder="e.g. Ahmed" autocomplete="given-name"><small>This is the friendly name shown on Home. You can change it later.</small></label><button class="primary full section" data-action="finish-name-setup">Continue</button></section></div>`}
function bindPreferredNameSetup(){const overlay=document.querySelector('.preferred-name-setup');if(!overlay)return;const input=overlay.querySelector('#setup-preferred-name'),btn=overlay.querySelector('[data-action="finish-name-setup"]');const save=async()=>{const name=String(input?.value||'').trim().slice(0,40);if(!name){input?.focus();return}state.profile.displayName=name;persist();try{await cloudApi.updateMetadata({preferred_name:name})}catch{}overlay.remove();render();announce(`Assalamualaikum, ${name}`)};btn?.addEventListener('click',save);input?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();save()}});setTimeout(()=>input?.focus(),30)}
function passwordRecoverySheet(){return `<div class="modal-backdrop password-recovery-sheet"><section class="sheet compact-sheet" role="dialog" aria-modal="true"><div class="sheet-head"><div><h2 class="title">Choose a new password</h2><p class="subtitle">Your recovery link was verified</p></div><button class="close" data-action="close-recovery-password">×</button></div><label class="mem-field"><span>New password</span><input id="new-password" type="password" class="search" minlength="6" autocomplete="new-password"></label><label class="mem-field"><span>Confirm password</span><input id="confirm-new-password" type="password" class="search" minlength="6" autocomplete="new-password"></label><button class="primary full section" data-action="save-new-password">Update password</button><div id="password-recovery-message" class="tiny muted section"></div></section></div>`}
function bindPasswordRecoverySheet(){const overlay=document.querySelector('.password-recovery-sheet');if(!overlay)return;overlay.querySelector('[data-action="close-recovery-password"]')?.addEventListener('click',()=>overlay.remove());overlay.querySelector('[data-action="save-new-password"]')?.addEventListener('click',async()=>{const p=overlay.querySelector('#new-password')?.value||'',c=overlay.querySelector('#confirm-new-password')?.value||'',msg=overlay.querySelector('#password-recovery-message');if(p.length<6){msg.textContent='Use at least 6 characters.';return}if(p!==c){msg.textContent='The passwords do not match.';return}try{const {error}=await cloudApi.updatePassword(p);if(error)throw error;passwordRecoveryMode=false;overlay.remove();announce('Password updated.')}catch(err){msg.textContent=err?.message||'Could not update password.'}})}
function importBackupFile(file,overlay){
  if(!file)return;const reader=new FileReader();reader.onload=async()=>{try{const parsed=JSON.parse(String(reader.result||'')),incoming=parsed?.state||parsed;if(!incoming||typeof incoming!=='object')throw new Error('Invalid backup');if(!confirm('Import this Quran Revision backup?\n\nYour current local data will be saved as a recovery snapshot first.'))return;captureRecoverySnapshot({force:true,label:'before-import'});state=normalizeIncomingState(incoming);saveState(state);markLocalUpdated();overlay?.remove();render();if(cloudSession)await syncCloudState({announceResult:true});else announce('Backup imported locally.')}catch{announce('That file is not a valid Quran Revision backup.')}};reader.readAsText(file)
}
function persist(){captureRecoverySnapshot();saveState(state);markLocalUpdated();scheduleCloudSync()}
...
function profileView(){
  const initial=(state.profile.displayName||cloudUserEmail()||'A').trim().charAt(0).toUpperCase()||'A';
  return `<div class="row" style="gap:10px"><div style="width:42px;height:42px;border-radius:50%;background:var(--accent-soft);display:grid;place-items:center;font-weight:800">${escapeHtml(initial)}</div><div><h1 class="title">Revision Profile</h1><p class="subtitle">Profile & settings</p></div></div><section class="card pad-sm section"><div class="between"><div><h2 class="section-title">Revision collection</h2><div class="small"><b>${state.collection.length} memorized Surahs</b></div><div class="tiny muted">Only these count toward revision stats</div></div><button class="secondary" data-action="open-add">Manage</button></div></section><section class="card pad-sm section"><div class="between"><div><h2 class="section-title">Account & Security</h2><p class="small muted" style="line-height:1.5;margin:5px 0 0">${escapeHtml(cloudStatusDescription())}</p></div><span class="account-status-pill ${cloudSession?'cloud-on':''}">${escapeHtml(cloudStatusLabel())}</span></div><button class="secondary full section" data-action="account-settings">Open account settings</button></section><section class="card pad-sm section recovery-backups"><div class="between"><div><h2 class="section-title">Recovery backups</h2><div class="tiny muted">Automatic local snapshots for accidental deletes or changes.</div></div><span class="recovery-shield">↶</span></div>${recoveryBackupCard(1,'About 1 hour ago')}${recoveryBackupCard(24,'About 24 hours ago')}<div class="tiny muted recovery-note">Restoring replaces the current local state. The app saves an emergency snapshot of your current data first.</div></section><section class="section" style="border-top:1px solid var(--border)"><button class="profile-row" data-action="account-settings"><span><b>Account & Security</b><small>${cloudSession?'Cloud sync, Google, password':'Sign in, Google, password'}</small></span><span>›</span></button><button class="profile-row" data-action="open-add"><span>Memorized Surahs</span><span>›</span></button><button class="profile-row" data-action="go-stats"><span>Revision history & notes</span><span>›</span></button><button class="profile-row" data-action="download-backup"><span><b>Download backup</b><small>Save a copy of your local app data now</small></span><span>↓</span></button><button class="profile-row" data-action="reset-demo"><span>Reset local prototype data</span><span>›</span></button></section>`;
}

function accountSecuritySheet(){
  const signedIn=!!cloudSession,email=cloudUserEmail(),googleLinked=!!cloudSession?.user?.identities?.some(i=>i?.provider==='google');
  const authBlock=signedIn?`<div class="cloud-account-card"><div class="between"><div><b class="small">Signed in</b><div class="tiny muted">${escapeHtml(email||'Supabase account')}</div></div><span class="account-status-pill cloud-on">${escapeHtml(cloudStatusLabel())}</span></div><div class="tiny muted cloud-sync-copy">${escapeHtml(cloudStatusDescription())}</div><div class="cloud-account-actions"><button class="secondary" data-action="sync-now">Sync now</button><button class="secondary danger-outline" data-action="cloud-sign-out">Sign out</button></div></div>`:`<div class="auth-entry-card"><button class="google-signin-btn" data-action="google-sign-in"><span class="google-mark"><svg class="google-g-svg" viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path fill="#4285F4" d="M17.64 9.205c0-.638-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 0 1-1.797 2.716v2.258h2.909c1.702-1.567 2.684-3.877 2.684-6.614z"/><path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.181l-2.909-2.258c-.806.54-1.835.859-3.047.859-2.344 0-4.328-1.584-5.037-3.711H.956v2.332A9 9 0 0 0 9 18z"/><path fill="#FBBC05" d="M3.963 10.709A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.281-1.709V4.959H.956A9 9 0 0 0 0 9c0 1.45.347 2.824.956 4.041l3.007-2.332z"/><path fill="#EA4335" d="M9 3.58c1.321 0 2.507.454 3.442 1.346l2.582-2.582C13.463.891 11.43 0 9 0A9 9 0 0 0 .956 4.959l3.007 2.332C4.672 5.164 6.656 3.58 9 3.58z"/></svg></span><b>Continue with Google</b></button><div class="auth-divider"><span>or use email</span></div><label class="auth-field"><span>Email</span><input id="auth-email" class="search" type="email" autocomplete="email" placeholder="you@example.com"></label><label class="auth-field"><span>Password</span><input id="auth-password" class="search" type="password" minlength="6" autocomplete="current-password" placeholder="At least 6 characters"></label><div class="auth-email-actions"><button class="primary" data-action="email-sign-in">Sign in</button><button class="secondary" data-action="email-sign-up">Create account</button></div><button class="auth-text-btn" data-action="send-password-reset">Forgot password?</button><div id="auth-message" class="tiny muted"></div></div>`;
  return `<div class="modal-backdrop"><section class="sheet account-sheet" role="dialog" aria-modal="true" aria-label="Account and security settings" data-sheet><div class="sheet-head"><div><h2 class="title">Account & Security</h2><p class="subtitle">${signedIn?'Private cloud sync is active':'Local-first · sign in when you’re ready'}</p></div><button class="close" data-action="close-sheet">×</button></div>
    ${authBlock}
    <div class="account-setting-card preferred-name-card"><div class="account-setting-icon">☺</div><div class="preferred-name-copy"><b class="small">What should we call you?</b><div class="tiny muted">This friendly name is shown on Home and syncs with your Quran Revision data.</div><input id="preferred-name" class="search preferred-name-input" maxlength="40" value="${escapeHtml(state.profile.displayName||'')}" placeholder="e.g. Ahmed"></div><button class="secondary" data-action="save-preferred-name">Save</button></div>
    ${signedIn?`<div class="account-setting-card"><div class="account-setting-icon google-g"><svg class="google-g-svg" viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path fill="#4285F4" d="M17.64 9.205c0-.638-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 0 1-1.797 2.716v2.258h2.909c1.702-1.567 2.684-3.877 2.684-6.614z"/><path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.181l-2.909-2.258c-.806.54-1.835.859-3.047.859-2.344 0-4.328-1.584-5.037-3.711H.956v2.332A9 9 0 0 0 9 18z"/><path fill="#FBBC05" d="M3.963 10.709A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.281-1.709V4.959H.956A9 9 0 0 0 0 9c0 1.45.347 2.824.956 4.041l3.007-2.332z"/><path fill="#EA4335" d="M9 3.58c1.321 0 2.507.454 3.442 1.346l2.582-2.582C13.463.891 11.43 0 9 0A9 9 0 0 0 .956 4.959l3.007 2.332C4.672 5.164 6.656 3.58 9 3.58z"/></svg></div><div><b class="small">Google account</b><div class="tiny muted">${googleLinked?'Google is linked to this account.':'Link Google so you can use Continue with Google for this same account.'}</div></div><button class="secondary" data-action="link-google" ${googleLinked?'disabled':''}>${googleLinked?'Linked':'Link'}</button></div>`:''}
    <div class="account-setting-card"><div class="account-setting-icon">⌁</div><div><b class="small">Reset password</b><div class="tiny muted">Send a secure recovery email${email?` to ${escapeHtml(email)}`:''}.</div></div><button class="secondary" data-action="send-password-reset">Reset</button></div>
    <div class="account-setting-card"><div class="account-setting-icon">⇄</div><div><b class="small">Transfer account data</b><div class="tiny muted">Copy or move data to another verified account. This safety flow is still being built.</div></div><button class="secondary" data-action="account-transfer-info">Info</button></div>
    <div id="account-feature-detail" class="notice section account-feature-detail"><b>Storage:</b> every change still saves locally first. ${signedIn?'Your private cloud row is also kept in sync.':'Sign in to add encrypted-in-transit cloud syncing across your devices.'}</div>
    <div class="account-transfer-safety section"><b class="small">Move your existing local data</b><div class="tiny muted">If you used the old file-based app, download its backup and import it here. Browser storage does not automatically move between file:// and pages.dev.</div><div class="cloud-account-actions"><button class="secondary" data-action="import-backup">Import backup</button><input id="import-backup-file" type="file" accept="application/json,.json" hidden><button class="secondary" data-action="download-backup">Download backup</button></div></div>
    <div class="account-transfer-safety section"><b class="small">Recovery backups</b><div class="tiny muted">Automatic local snapshots remain separate from cloud sync.</div><div class="recovery-sheet-list">${recoveryBackupCard(1,'About 1 hour ago')}${recoveryBackupCard(24,'About 24 hours ago')}</div></div>
  </section></div>`;
}

function downloadLocalBackup(){
  try{
    const payload={app:'Quran Revision',version:'0.1.51',exportedAt:new Date().toISOString(),state};
...
function bindAccountSecuritySheet(){
  ...
  const message=(text,bad=false)=>{const box=overlay.querySelector('#auth-message')||overlay.querySelector('#account-feature-detail');if(box){box.textContent=text;box.classList.toggle('auth-error',!!bad)}};
  const saveName=overlay.querySelector('[data-action="save-preferred-name"]');
  if(saveName)saveName.addEventListener('click',async()=>{const input=overlay.querySelector('#preferred-name');state.profile.displayName=String(input?.value||'').trim().slice(0,40);persist();if(cloudSession){try{await cloudApi.updateMetadata({preferred_name:state.profile.displayName})}catch{}}render();overlay.outerHTML=accountSecuritySheet();bindAccountSecuritySheet();announce(state.profile.displayName?`We’ll call you ${state.profile.displayName}.`:'Preferred name cleared.');});
  ...
  overlay.querySelector('[data-action="google-sign-in"]')?.addEventListener('click',async()=>{if(!cloudApi.available){message('Cloud sign-in is unavailable right now.',true);return}try{const {error}=await cloudApi.signInGoogle();if(error)throw error}catch(err){message(err?.message||'Could not start Google sign-in.',true)}});
  overlay.querySelector('[data-action="link-google"]')?.addEventListener('click',async()=>{try{const {error}=await cloudApi.linkGoogle();if(error)throw error}catch(err){message(err?.message||'Could not link Google.',true)}});
  const readAuth=()=>({email:String(overlay.querySelector('#auth-email')?.value||'').trim(),password:String(overlay.querySelector('#auth-password')?.value||'')});
  overlay.querySelector('[data-action="email-sign-in"]')?.addEventListener('click',async()=>{const {email,password}=readAuth();if(!email||!password){message('Enter your email and password.',true);return}message('Signing in…');try{const {error}=await cloudApi.signInEmail(email,password);if(error)throw error;overlay.remove();announce('Signed in.')}catch(err){message(err?.message||'Could not sign in.',true)}});
  overlay.querySelector('[data-action="email-sign-up"]')?.addEventListener('click',async()=>{const {email,password}=readAuth();if(!email||password.length<6){message('Enter an email and a password of at least 6 characters.',true);return}message('Creating account…');try{const {data,error}=await cloudApi.signUpEmail(email,password,state.profile.displayName);if(error)throw error;if(data?.session){overlay.remove();announce('Account created.')}else message('Account created. Check your email to confirm your address, then return here to sign in.')}catch(err){message(err?.message||'Could not create account.',true)}});
  overlay.querySelectorAll('[data-action="send-password-reset"]').forEach(btn=>btn.addEventListener('click',async()=>{const email=cloudUserEmail()||String(overlay.querySelector('#auth-email')?.value||'').trim()||prompt('Email address for password recovery:')||'';if(!email)return;message('Sending recovery email…');try{const {error}=await cloudApi.sendPasswordReset(email);if(error)throw error;message('Recovery email sent. Open its link to choose a new password.')}catch(err){message(err?.message||'Could not send the recovery email.',true)}}));
  overlay.querySelector('[data-action="sync-now"]')?.addEventListener('click',()=>syncCloudState({announceResult:true}));
  overlay.querySelector('[data-action="cloud-sign-out"]')?.addEventListener('click',async()=>{if(!confirm('Sign out of Quran Revision on this device?\n\nYour local copy will stay on this device.'))return;try{const {error}=await cloudApi.signOut();if(error)throw error;cloudSession=null;cloudStatus='signed-out';overlay.remove();render();announce('Signed out. Local data is still available.')}catch(err){message(err?.message||'Could not sign out.',true)}});
  overlay.querySelector('[data-action="account-transfer-info"]')?.addEventListener('click',()=>message('Account transfer will be added after the core sign-in and sync flow is fully tested. It will require verification on both accounts before any data moves.'));
  overlay.querySelector('[data-action="import-backup"]')?.addEventListener('click',()=>overlay.querySelector('#import-backup-file')?.click());
  overlay.querySelector('#import-backup-file')?.addEventListener('change',e=>importBackupFile(e.target.files?.[0],overlay));
...
ensureCatalog();render();if(state.mode==='listen')ensureReciters();if(state.mode==='practice'&&state.screen==='meanings')ensureMeaningBank();initCloudAuth();window.addEventListener('online',()=>{if(cloudSession)syncCloudState()});document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'&&cloudSession)scheduleCloudSync()});
